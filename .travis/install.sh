...
export PULP_RPM_PR_NUMBER=$(echo $COMMIT_MSG | grep -oP 'Required\ PR:\ https\:\/\/github\.com\/pulp\/pulp_\/pull\/(\d+)' | awk -F'/' '{print $7}')
...
if [ -z "$PULP_RPM_PR_NUMBER" ]; then
  pip install git+https://github.com/pulp/pulp_rpm.git#egg=pulp_rpm
else
  cd ../
  git clone https://github.com/pulp/pulp_file.git
  cd pulp_rpm
